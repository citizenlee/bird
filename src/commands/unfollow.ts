...
export function registerUnfollowCommand(program: Command, ctx: CliContext): void {
  program
    .command('unfollow')
    .description('Unfollow one or more users')
    .argument('<user-id-or-handle...>', 'User IDs or @handles to unfollow')
    ...
    .action(async (userIdOrHandles: string[], cmdOpts: { json?: boolean }) => {
      ...
      for (const input of userIdOrHandles) {
        ...
        if (result.success) {
          ...
          if (!cmdOpts.json) {
            console.log(`${ctx.p('ok')}Unfollowed ${displayName}${isUserId ? '' : ` (ID: ${userId})`}`);
          }
        } else {
          failures += 1;
        ...
        // Small delay between unfollows to avoid rate limiting
